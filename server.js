...
const basicAuth = require("express-basic-auth");   // <- если нужно, можно использовать
...
app.use(session({
  ...
  cookie: { secure: false } // для HTTPS — true
}));
...
// Дефолтные пути для логотипов (если поле пустое или содержит "none.png")
...
/**
 * Функция для выбора логотипа для команды.
 */
...
app.post("/api/matchdata", (req, res) => {
  ...
  // Сохраняем данные и уведомляем клиентов
  saveDataToFile();
...
// Если нужно, можно добавить fs.watch для index.html (в продакшене можно убрать)
