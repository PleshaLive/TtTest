...
const basicAuth = require("express-basic-auth"); // если нужно
...
// Middleware: если не залогинен, перенаправляем на /login (исключая /api и файлы логина)
...
// Дефолтные пути для логотипов (если не указан или содержит "none.png")
...
// Функция загрузки данных из файла db.json
...
// Функция сохранения данных в файл db.json
...
app.post("/api/matchdata", (req, res) => {
  ...
  // Сохраняем данные и уведомляем клиентов с задержкой
  saveDataToFile();
...
// Если требуется, можно добавить наблюдение за index.html (для live reload в разработке)
